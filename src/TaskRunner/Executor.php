...
use MateCat\Commons\Database;
use MateCat\Commons\Log;
use MateCat\Commons\Utils;
use MateCat\TaskRunner\Config\Bootstrap;
use MateCat\TaskRunner\Config\INIT;
use MateCat\TaskRunner\Commons\AbstractWorker;
use MateCat\TaskRunner\Commons\Context;
use MateCat\TaskRunner\Commons\QueueElement;
use MateCat\TaskRunner\Exceptions\EmptyElementException;
use MateCat\TaskRunner\Exceptions\EndQueueException;
use MateCat\TaskRunner\Exceptions\FrameException;
use MateCat\TaskRunner\Exceptions\ReQueueException;
use MateCat\TaskRunner\Exceptions\WorkerClassException;
use Exception;
use PDOException;
use RedisException;
use SplObserver;
...
//            $this->_logMsg( "--- (Worker " . $this->_executor_instance_id . ") - QueueElement found: " . var_export( $queueElement, true ) );
            ...
            try {
                 ...
                 */
                if ( empty( $this->_worker ) || ltrim( $queueElement->classLoad, "\\" ) != ltrim( get_class( $this->_worker ), "\\" ) ) {
...
//                $this->_logMsg( $e->getMessage() );
    ...
    public static function cleanShutDown() {
        ...
        static::$__INSTANCE->_queueHandler->getRedisClient()->close();
